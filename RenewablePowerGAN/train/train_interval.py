"""
对于判别器过强的问题, 采取特别的trick
训练多次Generator, 只训练一次Discriminator, 同时使用预训练的Generator
Discriminator的学习率较小, Generator的学习率较大
...
def train(gen, disc, opt_gen, opt_disc, train_loader, criterion, epoch, interval=5):
...
def main():
    ...
    if config.TRAINED_MODEL:
        checkpoints = torch.load('./logs/[11-12]22.58.47/model_19800.tar')
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        # train for epoch
        dis_loss, gen_loss, real_score, fake_score = train(generator, discriminator, opt_gen, opt_disc, train_loader, criterion, epoch, interval=1)
